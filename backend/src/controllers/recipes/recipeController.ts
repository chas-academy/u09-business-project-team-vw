...
export const getOrFetchRecipe = async (req: Request, res: Response): Promise<void> => {
    const { id } = req.params;
    ...
    try {
        ...
        res.status(200).json(successResponse('Recipe fetched from API', recipeToSend));
        return;
...
export const saveRecipe = async (req: Request, res: Response) => {
    ...
    try {
        ...
        // Check if the recipe is already stored in the database
        if(existingRecipe) {
            res.status(200).json(successResponse('Recipe allready saved in DB', existingRecipe));
            return;
        }
        ...
        res.status(201).json(successResponse('Recipe saved', newRecipe));
        return;
